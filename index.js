...
app.put("/findOneAndUpdate", async (req, res) => {
  await Contact.findByIdAndUpdate("612df3bbeb561e1e783cf727", {
    name: "jaouher",
  });
});
...
app.delete("/emoveById,", async (req, res, done) => {
  const contact = await Contact.findByIdAndRemove(contactId, (err, data) =>
...
app.get("/queryCain", async (req, res, done) => {
  Contact.find({ favoriteFoods: foodToSearch })
    .sort({ name: "desc" })
    .limit(2)
    .select("-age")
    .exec((err, data) => {
